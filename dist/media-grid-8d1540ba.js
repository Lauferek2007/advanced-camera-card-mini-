import { eD as commonjsGlobal, fc as getDefaultExportFromCjs, eu as throttle, $ as getChildrenFromElement, z as isEqual, w as fireAdvancedCameraCardEvent, dW as dispatchExistingMediaLoadedInfoAsEvent, dX as dispatchMediaUnloadedEvent, I as setOrRemoveAttribute, ek as setOrRemoveStyleProperty, _ as __decorate, y as n, t, u as s, Y as e, C as x, a0 as n$1, v as r } from './card-edc26888.js';
...
class MediaGridController {
    constructor(host, options) {
        this._mediaLoadedInfoMap = new Map();
        this._gridContents = new Map();
        this._masonry = null;
        this._displayConfig = null;
        this._throttledLayout = throttle(() => this._masonry?.layout?.(), 
        ...
        (_mutations, _observer) => this._calculateGridContentsFromHost());
        this._cellMutationObserver = new MutationObserver(
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        (_mutations, _observer) => this._calculateGridContentsFromHost());
        this._hostResizeObserver = new ResizeObserver(this._hostResizeHandler.bind(this));
        this._cellResizeObserver = new ResizeObserver(this._cellResizeHandler.bind(this));
        this._calculateGridContentsFromHost = () => {
            const children = getChildrenFromElement(this._host);
            const gridContents = new Map();
        ...
        this._handleMediaLoadedInfoEvent = (ev) => {
        ...
        this._handleSelectGridCellEvent = (ev) => {
            const eventPath = ev.composedPath();
            for (const [id, element] of this._gridContents.entries()) {
                /* istanbul ignore else: the else path cannot be reached -- @preserve */
                if (eventPath.includes(element)) {
    ...
    _sortItemsInGrid() {
        ...
        const selectedItem = existingItems?.find((item) => item.element.getAttribute(this._idAttribute) === this._selected);
        // If `grid_selected_position` is set to 'first' or 'last', move the
        // selected item to the start or end of the list respectively.
        if (!!this._displayConfig?.grid_selected_position &&
            ...
            const newItems = this._displayConfig.grid_selected_position === 'first'
    ...
    _addChildEventListeners(child) {
        ...
        child.addEventListener('advanced-camera-card:media:loaded', this._handleMediaLoadedInfoEvent);
    }
    _removeChildEventListeners(child) {
        child.removeEventListener('click', this._handleSelectGridCellEvent, {
            capture: true,
        });
        child.removeEventListener('advanced-camera-card:media:loaded', this._handleMediaLoadedInfoEvent);
    }
    _createMasonry() {
        if (this._masonry) {
            this._masonry.destroy?.();
    ...
    _updateSelectedStylesOnElements() {
        for (const [id, element] of this._gridContents.entries()) {
    ...
    _updateWidthFactorStyles() {
        for (const element of this._gridContents.values()) {
            const widthFactor = element.getAttribute(this._widthFactorAttribute);
            setOrRemoveStyleProperty(element, !!widthFactor, '--advanced-camera-card-grid-width-factor', widthFactor ?? undefined);
        }
    }
    _getColumnSize() {
    ...
    _getColumns() {
        ...
        const idealColumns = Math.min(maxColumns, Math.floor(this._hostWidth / MEDIA_GRID_DEFAULT_IDEAL_CELL_WIDTH));
        if (idealColumns > 1) {
            return idealColumns;
        }
        // If not, get a multi-column view using the minimum cell width.
        const minColumns = Math.floor(Math.min(maxColumns, this._hostWidth / MEDIA_GRID_DEFAULT_MIN_CELL_WIDTH));
        // Last result use at least 1 column.
        return Math.max(1, minColumns);
    }
    _setColumnSizeStyles() {
        this._host.style.setProperty('--advanced-camera-card-grid-column-size', `${this._getColumnSize()}px`);
        this._host.style.setProperty('--advanced-camera-card-grid-selected-width-factor', `${this._displayConfig?.grid_selected_width_factor ??
            MEDIA_GRID_DEFAULT_SELECTED_WIDTH_FACTOR}`);
    }
}

var css = ":host {\n  display: block;\n  width: 100%;\n  height: 100%;\n  --advanced-camera-card-grid-border-size: 3px;\n  --advanced-camera-card-grid-column-size: 100%;\n  --advanced-camera-card-grid-selected-width-factor: 2;\n  overflow: auto;\n  scrollbar-width: none;\n  -ms-overflow-style: none;\n}\n\n/* Hide scrollbar for Chrome, Safari and Opera */\n:host::-webkit-scrollbar {\n  display: none;\n}\n\n::slotted(*) {\n  box-sizing: border-box;\n  --advanced-camera-card-grid-width-factor: 1;\n  width: calc(var(--advanced-camera-card-grid-width-factor) * var(--advanced-camera-card-grid-column-size));\n  border: var(--advanced-camera-card-grid-border-size) solid transparent;\n}\n\n::slotted([selected]) {\n  border: var(--advanced-camera-card-grid-border-size) solid var(--advanced-camera-card-grid-selected-border-color);\n  width: min(100%, var(--advanced-camera-card-grid-width-factor) * var(--advanced-camera-card-grid-selected-width-factor) * var(--advanced-camera-card-grid-column-size));\n}\n\nslot {\n  display: block;\n}";

let AdvancedCameraCardMediaGrid = class AdvancedCameraCardMediaGrid extends s {
    constructor() {
        super(...arguments);
    ...
    render() {
        return x `<slot ${n$1(this._refSlot)}></slot> `;
    }
    static get styles() {
        return r(css);
    }
...
__decorate([
    n({ attribute: false })
], AdvancedCameraCardMediaGrid.prototype, "selected", void 0);
__decorate([
    n({ attribute: false })
], AdvancedCameraCardMediaGrid.prototype, "displayConfig", void 0);
AdvancedCameraCardMediaGrid = __decorate([
    t('advanced-camera-card-media-grid')
], AdvancedCameraCardMediaGrid);

export { AdvancedCameraCardMediaGrid };
//# sourceMappingURL=media-grid-8d1540ba.js.map
