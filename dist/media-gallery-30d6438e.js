import{d$ as e,Q as a,B as t,e0 as r,e1 as i,e2 as n,M as s,_ as o,y as l,t as d,u as c,C as h,J as m,a4 as u,l as g,v as y}from"./card-c2e90b76.js";import"./gallery-core-3e9fd108.js";import"./card-5a8ce8cf.js";import"./endOfDay-f911d86b.js";import"./date-picker-736387fa.js";import"./until-ec1347cc.js";class v{constructor(e){this._media=null,this._host=e}getMedia(){return this._media}setMediaFromView(e,a){const t=e?.queryResults?.getResults()??null;null!==t?this._media&&a?.queryResults?.getResults()===t||(this._media=[...t].reverse()):this._media=null}setThumbnailSize(a){this._host.style.setProperty("--advanced-camera-card-mini-thumbnail-size",`${a??e}px`)}getColumnWidth(a){return a?a.show_details?300:a.size:e}getColumnCountRoundMethod(e){return e?.show_details?"floor":"ceil"}async extendMediaGallery(e,s,o,l=!0){const d=s.manager.getView();if(!d)return;const c=d.query,h=d.queryResults?.getResults();if(!h||!c||!a.isMediaQuery(c))return;const m=c.getQuery()??null;if(!m)return;let u;try{u=await e.extendMediaQueries(m,h,o,{useCache:l})}catch(e){return void t(e)}if(u){const e=a.isEventQuery(c)?new r(u.queries):a.isRecordingQuery(c)?new i(u.queries):/* istanbul ignore next: this path cannot be reached -- @preserve */
null;
/* istanbul ignore else: this path cannot be reached, as we explicitly
      ...
      ${this._controller.getMedia()?.map(((a,t)=>h`<advanced-camera-card-mini-thumbnail
            class=${m({selected:a===e})}
            .hass=${this.hass}
    ...
    `}render(){const e=!!this.viewManagerEpoch?.manager.getView()?.context?.loading?.query;return h`
      ...
      >
        ${this.galleryConfig&&"none"!==this.galleryConfig.controls.filter.mode?h` <advanced-camera-card-mini-media-filter
              .hass=${this.hass}
              .cameraManager=${this.cameraManager}
              .viewManagerEpoch=${this.viewManagerEpoch}
            ...
            >
            </advanced-camera-card-mini-media-filter>`:""}
        ${this._controller.getMedia()?.length?h`<advanced-camera-card-mini-gallery-core
              .hass=${this.hass}
